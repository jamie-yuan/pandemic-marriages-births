...
from compute_on_data import calculate_marriage_sinusoid, calculate_birth_sinusoid, \
    marriage_difference, birth_difference
...
def format_graph_data() -> dict[str, list]:
    ...
    return {'dates': list(birth_data().keys()),
            ...
            'marriages sinusoid': format_marriage_sinusoid_data(),
            'births sinusoid': format_birth_sinusoid_data()}


def format_marriage_sinusoid_data() -> list[float]:
    """Format the sinusoid data approximating the marriage data to be used in Plotly by the
    graph_data function.
    """
    ...
    marriage_sinusoid = []
    for x in range(36):
        marriage_sinusoid.append((a * math.sin(b * (x - c)) + d) + 1750)  # adjust based on the
        # graph of the curve

    return marriage_sinusoid


def format_birth_sinusoid_data() -> list[float]:
    """Format the sinusoid data approximating the birth data to be used in Plotly by the
    graph_data function.
    """
    ...
    birth_sinusoid = []
    for x in range(36):
        birth_sinusoid.append((a * math.sin(b * (x - c)) + d) + 1000)  # adjust based on the
        # graph of the curve

    return birth_sinusoid
